...
@manager.command
def restore(path_or_filename):
    """Restore db

    Given a path or filename, restore db. If the str is path formatted, looks
    for file at that path. If it is a filename, looks for local file in
    default backups folder, and if finds none, looks for file on the cloud.

    Args:
        path_or_filename (str): Path to backup file, or simply a filename
    """
    restore_db(path_or_filename)
