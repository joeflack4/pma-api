...
backup_created_on_diff_os_tell = 'invalid locale name'
        ...
        # Print error if DB background process is not loaded
            ...
            if "'NoneType' object has no attribute 'drivername'" in str(err):
                msg = (
                    ...
                    'accessed. Please check that they are set and being loaded '
                    'correctly.\n\n'
                    '- Original error:\n'
                    + type(err).__name__ + ': ' + str(err))
            else:
...
def new_backup_path(ext: str = 'dump'):
    ...
    datetime_str = str(datetime.now()).replace('/', '-').replace(':', '-')\
        .replace(' ', '_')
    filename = '_'.join([filename_base, platform.system(), datetime_str]) \
               + '.' + ext
...
def backup_using_pgdump_gz(path: str = new_backup_path(ext = 'gz')):
    ...
    cmd = 'pg_dump --no-owner -h localhost -U postgres {}'\
    ...
    if errors:
        msg = 'Error occurred while executing command: {}'.format(' '.join(cmd))
        raise PmaApiDbInteractionError(
            msg +
            '\nOriginal error:\n' +
            errors)
...
def backup_using_pgdump_dump(path: str = new_backup_path(ext = 'dump'),
                             attempt_num: int = 1,
                             running_errs: list = []) -> str:
    """Backup using pg_dump

    Args:
        path (str): Path of file to save
        attempt_num (int): Number of tries attempted
        running_errs (list[str]): Any errors that occurred during attempts
    ...
    Returns:
        str: path to backup file saved
    """
    version_variant_commands = [
        'pg_dump --no-owner --format=custom --dbname={} --file={}',
        'pg_dump -O -Fc -d {} -f {}',
        'pg_dump -O -Fc {} {}',
        # 'pg_dump --no-owner -Fc --dbname={} {}',
        # 'pg_dump -Fc {} {}',
        # 'pg_dump -Fc --dbname={} {}',
        # 'pg_dump --no-owner -Fc {} --file {}',
        # 'pg_dump -Fc --dbname={} --file={}',
        # 'pg_dump -Fc --dbname={} --file={}',
        # 'pg_dump --dbname={} --file={}'
    ]
    max_attempts = len(version_variant_commands)
    cmd = version_variant_commands[attempt_num - 1]\
    ...
    errors = proc.stderr.read()
    if errors:
        running_errs.append(errors)
        if attempt_num < max_attempts:
            backup_using_pgdump_dump(
                path=path,
                attempt_num=attempt_num + 1,
                running_errs=running_errs)
        else:
            msg = 'Errors occurred while executing backup using pg_dump. The ' \
                  'following attempts were made, and resulted in errors as ' \
                  'follows: '
            for i, cmd in enumerate(version_variant_commands):
                attempt = i + 1
                cmd2 = cmd.format(Config.DATABASE_NAME, path)
                msg += '\nCommand {}: {}'.format(attempt, cmd2)
                msg += '\nError {}: {}'.format(attempt, running_errs[i])
            raise PmaApiDbInteractionError(msg)
...
# noinspection PyDefaultArgument
def restore_using_pgrestore(path: str, attempt_num: int = 1,
                            running_errs: list = []):
    ...
    version_variant_commands = [
        'pg_restore --create --no-owner --username={} --dbname postgres {}'
        .format('postgres', path),
        # 'pg_restore --create --no-owner --username={} --dbname postgres {}'
        # .format(os.getenv('DATABASE_USER', 'pmaapi'), path),
        # 'pg_restore --create --no-owner --username={} -d postgres {}'
        # .format(os.getenv('DATABASE_USER', 'pmaapi'), path),
        # 'pg_restore -C -d postgres {}'.format(path),
        # 'pg_restore --create --username={} -d postgres {}'
        # .format(os.getenv('DATABASE_USER', 'pmaapi'), path),
    ...
    if errors:
        ...
        if 'could not open input file' in errors \
        ...
        elif backup_created_on_diff_os_tell in errors:
            msg = 'Errors occurred while executing restore using ' \
                  'pg_restore. The type of error received indicates that the ' \
                  'root caues of the issue is that the current operating ' \
                  'system ({}) is different than the one which was used to ' \
                  'create the original backup file. You may be able to find ' \
                  'more information here: ' \
                  'https://wiki.postgresql.org/wiki/Usability_Challenges'\
            ...
            if attempt_num < max_attempts:
                ...
                msg = 'Errors occurred while executing restore using ' \
                      'pg_restore. The following attempts were made, and ' \
...
def drop_db(db_name: str = Config.DATABASE_NAME):
    ...
    errors = proc.stderr.read()
    if errors:
        db_not_exist = True if db_not_exist_tell in errors else False
        db_exists = not db_not_exist
        if db_exists:
            log_process_stderr(proc.stderr, err_msg=db_mgmt_err)
            msg = db_mgmt_err + '\n\n  - Original command: ' + ' '.join(cmd)
            raise PmaApiDbInteractionError(
                msg +
                '\n  - Original error:\n' +
                errors)
